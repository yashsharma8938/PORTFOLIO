import { Suspense, lazy, useRef, useCallback, memo } from 'react'
import { motion } from 'framer-motion'
import { useCursorPosition } from '@/hooks/useCursorPosition'

const Scene = lazy(() => import('@/components/3d/Scene').then(m => ({ default: m.Scene })))

// Memoized static sections to prevent unnecessary re-renders
...
export function Hero() {
  // Cursor glow via direct DOM manipulation — zero re-renders
  const glowRef = useRef<HTMLDivElement>(null)

  const onCursorMove = useCallback((pos: { x: number; y: number }) => {
    if (glowRef.current) {
      glowRef.current.style.left = `${pos.x}px`
      glowRef.current.style.top = `${pos.y}px`
    }
  }, [])

  useCursorPosition(onCursorMove)
  ...
  return (
    ...
    >
      ...
      {/* Cursor glow — moved via ref, no setState */}
      <div
        ref={glowRef}
