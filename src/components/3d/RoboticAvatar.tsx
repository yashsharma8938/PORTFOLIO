...
function RoboticAvatarInner({ mouseX, mouseY, laserActive = false }: RoboticAvatarProps) {
  ...
  // Read cursor from mutable ref — no re-renders on mouse move
  ...
  useFrame((state) => {
    ...
    if (headRef.current) {
      headRef.current.rotation.y = THREE.MathUtils.lerp(headRef.current.rotation.y, mx * 0.45, 0.06)
      headRef.current.rotation.x = THREE.MathUtils.lerp(headRef.current.rotation.x, -my * 0.28, 0.06)
    }
    ...
    if (leftArmRef.current) leftArmRef.current.rotation.z = Math.sin(t * 0.8) * 0.06 + 0.1
    if (rightArmRef.current) rightArmRef.current.rotation.z = -Math.sin(t * 0.8) * 0.06 - 0.1

    if (groupRef.current) {
      groupRef.current.rotation.y = THREE.MathUtils.lerp(groupRef.current.rotation.y, mx * 0.08, 0.03)
    }
  ...
  // ── Cached materials — created once, never re-allocated ──
  const chrome = useMemo(() => ({
    color: '#c0c2cc', metalness: 0.97, roughness: 0.05, envMapIntensity: 2.5,
  }), [])
  const gunmetal = useMemo(() => ({
    color: '#1c1e26', metalness: 0.92, roughness: 0.18, envMapIntensity: 1.8,
  }), [])
  const visorBlue = useMemo(() => ({
    color: '#1a4fff', metalness: 0.3, roughness: 0.1,
    emissive: '#0030cc' as unknown as THREE.Color,
    emissiveIntensity: 0.8, transparent: true, opacity: 0.85,
  }), [])
  const accent = useMemo(() => ({
    color: '#4F8EF7', metalness: 0.4, roughness: 0.1,
    emissive: '#234acc' as unknown as THREE.Color, emissiveIntensity: 0.5,
  }), [])
    ...
    <group ref={groupRef} position={[2.6, -0.8, 0]} scale={0.82}>
      <group ref={bodyRef}>
        ...
        <mesh position={[0, 0.18, 0.24]}>
          ...
          <meshStandardMaterial color="#4F8EF7" emissive="#4F8EF7" emissiveIntensity={2} metalness={0.2} roughness={0.1} />
        </mesh>
        <mesh position={[0, 0.18, 0.23]}>
          <torusGeometry args={[0.09, 0.012, 8, 32]} />
          <meshStandardMaterial color="#6ec6ff" emissive="#4F8EF7" emissiveIntensity={1.5} transparent opacity={0.8} />
        </mesh>
        ...
        <group ref={headRef} position={[0, 0.82, 0]}>
          ...
          <mesh position={[0, 0.04, 0.2]}>
            ...
            <meshStandardMaterial
              color="#4F8EF7" emissive="#4F8EF7" emissiveIntensity={0.4}
              metalness={0} roughness={1} transparent opacity={0.3}
            />
          </mesh>
          ...
          <mesh ref={leftEyeRef} position={[-0.1, 0.06, 0.245]}>
            ...
            <meshStandardMaterial color="#00aaff" emissive="#0066ff" emissiveIntensity={0.8} />
          </mesh>
          ...
          <group ref={laserGroupRef}>
            <mesh ref={laserLeftRef} position={[-0.55, 0.06, 0.245]} rotation={[0, Math.PI / 2, 0]}>
              <cylinderGeometry args={[0.004, 0.004, 1.0, 8]} />
              <meshStandardMaterial color="#4488ff" emissive="#4488ff" emissiveIntensity={0} transparent opacity={0} depthWrite={false} />
            </mesh>
            <mesh ref={laserRightRef} position={[-0.35, 0.06, 0.245]} rotation={[0, Math.PI / 2, 0]}>
              <cylinderGeometry args={[0.004, 0.004, 1.0, 8]} />
              <meshStandardMaterial color="#4488ff" emissive="#4488ff" emissiveIntensity={0} transparent opacity={0} depthWrite={false} />
            </mesh>
          </group>
          ...
          <mesh position={[0.15, 0.5, 0]}>
            <sphereGeometry args={[0.025, 12, 12]} />
            <meshStandardMaterial color="#4F8EF7" emissive="#4F8EF7" emissiveIntensity={3} />
          </mesh>
        </group>
      ...
      <mesh ref={floorGlowRef} position={[0, -1.56, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial color="#4F8EF7" emissive="#4F8EF7" emissiveIntensity={0.3} transparent opacity={0.15} />
      </mesh>
    </group>
  )
}

export const RoboticAvatar = memo(RoboticAvatarInner)
